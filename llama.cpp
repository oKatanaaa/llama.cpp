...
bool llama_model_load_fast(const std::string & fname, llama_model & model, gpt_vocab & vocab, int n_ctx)  {
    ...
    if (fstat(fileno(fin), &stats) == -1) {
        fprintf(stderr, "%s: failed to read stats '%s'\n", __func__, fname.c_str());
        return false;
    }

    fprintf(stderr, "%s: optimal buffer size is %d bytes, BUFSIZ = %d\n", __func__, stats.st_blksize, BUFSIZ);
